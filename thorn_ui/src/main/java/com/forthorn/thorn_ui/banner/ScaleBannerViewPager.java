package com.forthorn.thorn_ui.banner;
 ...
 * Date: 4/2/2018.
 * Description:
 * 带缩放效果的BannerViewPager
 * 配合Banner使用
 * Banner使用自定义的布局，在banner_layout中使用自定义布局
 *
 */

public class ScaleBannerViewPager extends BannerViewPager {
    ...
    private float mScaleMin = 0.857f;
    private float mCoverWidth = 0f;

    public ScaleBannerViewPager(Context context) {
        super(context);
        init(context, null);
    }

    public ScaleBannerViewPager(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    private void init(Context context, AttributeSet attrs) {
        setPageTransformer(true, new ScaleBannerViewPager.SPageTransformer());//默认调用了 setChildrenDrawingOrderEnabledCompat(true);使得getChildDrawingOrder起作用
    ...
    class SPageTransformer implements ViewPager.PageTransformer {
        ...
        public void transformPage(View view, float position) {
            if (offsetPosition == 0f) {
                float paddingLeft = ScaleBannerViewPager.this.getPaddingLeft();
                float paddingRight = ScaleBannerViewPager.this.getPaddingRight();
                float width = ScaleBannerViewPager.this.getMeasuredWidth();
